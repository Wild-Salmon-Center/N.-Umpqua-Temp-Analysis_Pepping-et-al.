...
### Air Logger IDs:
#### 21433138 - Copeland Cr.
#### 21679162 - N. Umpqua at Steamboat Cr.
...
pacman::p_load(tidyverse, readxl, janitor, writexl)
```
...
```{r Define functions for regression analysis}

# Air logger look up function 
air_logger <- function(log_id){
  daily_air_master %>% 
  filter(logger_id == log_id) %>% 
  mutate(longdate = make_date(year,month,day)) %>% 
  filter(longdate > '2024-06-19' & longdate <= '2024-09-15') %>% 
  mutate(air_mean = mean_temp) 
}

# Water logger look up function
water_logger <- function(log_id) {
  daily_water_master %>% 
  filter(logger_id == log_id) %>% 
  mutate(longdate = make_date(year,month,day)) %>%
  filter(longdate > '2024-06-19' & longdate <= '2024-09-15') %>% 
...
# Join air & water readings function
logger_join <- function() {
  water %>% 
  left_join(air, by = "longdate")
}

# Regression equation function for plots
lm_eqn <- function(df){
...
# Function to predict future water temperature scenario and estimate # of days above 20C (summer season)
future_climate_predict <- function(df) {
  new_temps = data.frame(air_mean = airwater_join$air_mean + 3.5) # Create vector of new air temps under 3.5C warming scenario
  fit = lm(water_max ~ air_mean, data = airwater_join) # Define lm
  watertemp_pred = predict(fit, newdata = new_temps) # Use lm to predict new water temps using updated air temps
  daysover20_future = data_frame(daysover20 = sum(watertemp_pred > 20)) # Estimate number of days water temp above 20C given future climate scenario 
  print(paste("Predicted # days above 20C (+3.5C future climate):", {daysover20_future$daysover20})) # Print results 
}
```
